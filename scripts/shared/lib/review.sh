...
run_repo_review() {
  ...
  review_run_command failures "顶层场景帮助可读性" bash "${scripts_root}/manage.sh" scenario --help
  review_run_command failures "顶层配置写入路由回归" bash -lc "tmp='${probe_root}/conf/root-config-set.conf' && cp '${scripts_root}/coding-agents/codex/conf.example' \"\${tmp}\" && bash '${scripts_root}/manage.sh' config set --config \"\${tmp}\" --set MODE=advanced --set PROVIDER_NAME=openai >/dev/null && rg -q '^MODE=advanced$' \"\${tmp}\" && rg -q '^PROVIDER_NAME=openai$' \"\${tmp}\""
  ...
  review_run_command failures "Codex Base URL 脱敏回归" bash -lc "tmp='${probe_root}/conf/codex-mask.conf' && cp '${scripts_root}/coding-agents/codex/conf.example' \"\${tmp}\" && printf '\nAPI_BASE_URL=https://private.example.internal/v1\n' >> \"\${tmp}\" && output=\"\$(bash '${scripts_root}/manage.sh' config show --tool-name codex --config \"\${tmp}\" --section summary)\" && printf '%s\n' \"\${output}\" | rg -q '已声明（已脱敏）' && ! printf '%s\n' \"\${output}\" | rg -q 'private\\.example\\.internal'"
  review_run_command failures "Claude Code Base URL 脱敏回归" bash -lc "tmp='${probe_root}/conf/claude-mask.conf' && cp '${scripts_root}/coding-agents/claude-code/conf.example' \"\${tmp}\" && printf '\nANTHROPIC_BASE_URL=https://private.example.internal/anthropic\n' >> \"\${tmp}\" && output=\"\$(bash '${scripts_root}/manage.sh' config show --tool-name claude-code --config \"\${tmp}\" --section summary)\" && printf '%s\n' \"\${output}\" | rg -q '已声明（已脱敏）' && ! printf '%s\n' \"\${output}\" | rg -q 'private\\.example\\.internal'"
  review_run_command failures "OpenCode Base URL 脱敏回归" bash -lc "tmp='${probe_root}/conf/opencode-mask.conf' && cp '${scripts_root}/coding-agents/opencode/conf.example' \"\${tmp}\" && sed -i 's|^OLLAMA_BASE_URL=.*|OLLAMA_BASE_URL=https://private.example.internal:11434|' \"\${tmp}\" && output=\"\$(bash '${scripts_root}/manage.sh' config show --tool-name opencode --config \"\${tmp}\" --section summary)\" && printf '%s\n' \"\${output}\" | rg -q '已声明（已脱敏）' && ! printf '%s\n' \"\${output}\" | rg -q 'private\\.example\\.internal'"
  review_run_command failures "OpenClaw 飞书敏感项脱敏回归" bash -lc "tmp='${probe_root}/conf/openclaw-mask.conf' && cp '${scripts_root}/general-agents/openclaw/conf.example' \"\${tmp}\" && printf '\nENABLE_FEISHU=1\nFEISHU_APP_ID=cli_aabbcc\nFEISHU_APP_SECRET=secret-value-123\nFEISHU_VERIFICATION_TOKEN=verify-value-123\nFEISHU_ENCRYPT_KEY=encrypt-value-123\n' >> \"\${tmp}\" && output=\"\$(bash '${scripts_root}/manage.sh' feishu check --tool-name openclaw --config \"\${tmp}\")\" && printf '%s\n' \"\${output}\" | rg -q 'App Secret' && printf '%s\n' \"\${output}\" | rg -q '已配置（已脱敏）' && ! printf '%s\n' \"\${output}\" | rg -q 'secret-value-123|verify-value-123|encrypt-value-123|cli_aabbcc'"
  ...
  review_require_pattern failures "场景向导使用敏感提示承接 Codex Base URL" "scene_prompt_sensitive_with_default \"API Base URL" "${scripts_root}/manage.sh"
  review_require_pattern failures "场景向导使用敏感提示承接 Claude Base URL" "scene_prompt_sensitive_with_default \"Anthropic Base URL" "${scripts_root}/manage.sh"
  review_require_pattern failures "场景向导使用敏感提示承接 OpenCode Base URL" "scene_prompt_sensitive_with_default \"Ollama Base URL" "${scripts_root}/manage.sh"
  review_require_pattern failures "场景向导声明 URL 仅在交互阶段明文显示" "仅当前交互明文显示" "${scripts_root}/manage.sh"
  ...
  review_reject_pattern failures "禁止旧的 .runtime 路径概念" '\.runtime/' "${scripts_root}/README.md" "${scripts_root}/ARCHITECTURE.md" "${scripts_root}/TASKS.md" "${scripts_root}/coding-agents/README.md" "${scripts_root}/coding-agents/codex/README.md" "${scripts_root}/coding-agents/claude-code/README.md" "${scripts_root}/coding-agents/gemini-cli/README.md" "${scripts_root}/coding-agents/opencode/README.md" "${scripts_root}/general-agents/README.md" "${scripts_root}/general-agents/openclaw/README.md"
